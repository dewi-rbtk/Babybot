...
async def _(hazmat):
    await hazmat.edit("`Baby Ingin Mengaktifkan Perintah Hazmat ヅ`")
    level = hazmat.pattern_match.group(2)
    if hazmat.fwd_from:
        return
    if not hazmat.reply_to_msg_id:
        await hazmat.edit("`Baby, Mohon Balas Ke Sticker/Gambar ヅ`")
        return
    reply_message = await hazmat.get_reply_message()
    if not reply_message.media:
        await hazmat.edit("`Kita Bisa Menghancurkan Apapun Sayang ヅ`")
        return
    ...
    async with hazmat.client.conversation(chat) as conv:
        try:
            ...
            await hazmat.reply("`Sayang, Mohon Buka Blokir` @hazmat_suit_bot `Lalu Coba Lagi`")
            return
        if response.text.startswith("I can't"):
            await hazmat.edit("`Mohon Maaf Sayang, GIF Tidak Bisa...`")
